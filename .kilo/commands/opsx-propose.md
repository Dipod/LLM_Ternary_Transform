...
- **Ask the user now, do not defer to apply.** The upstream OpenSpec default "prefer making reasonable decisions to keep momentum" is **overridden** for this project. If a decision is architecturally meaningful and ambiguous (module layout / library choice / dataset or model selection / dtype and precision policy / settings storage / caching or streaming strategy / error-handling pattern / logging strategy / Python and PyTorch version floor / public module signatures / gate thresholds), ask the user **here**, not at apply time.
- **Do not ask the user about facts that a run or an official document can close.** Library APIs and signatures, tensor shapes, dtype behaviour, model config fields, dataset schemas, metric definitions — resolve by executing a minimal snippet or reading the upstream documentation before reaching for `AskUserQuestion`.
...
- **The full rule lives in `.kilo/rules/sdd-integrations.md → Workflow` (propose phase). Load it before authoring any non-trivial proposal.**
...
- **Ask the user about every ambiguous architectural / scope / naming / placement / storage / library / dataset / model / dtype / gate-threshold decision during this phase — do NOT defer to apply.** "Prefer making reasonable decisions to keep momentum" is **not** the rule for this project.
