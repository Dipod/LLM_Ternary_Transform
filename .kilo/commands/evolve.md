...
Proposals that **weaken** any of the following are safety-relevant: the gate chain (`ruff check` / `mypy` / `pytest`, budgets, the `.kilo/rules/verification-gates.md` gates), experiment reproducibility and gate thresholds, secrets / credentials / PII handling, `CONFUSION` gates on material forks, the evidence rules of `.kilo/rules/research-discipline.md`.
...
- prefer narrowing the scope or introducing a setting (`QUICKFIX_MAX_LINES`, a `.dev.env` key) over deleting or relaxing the rule text;
...
Entry format (English, imperative, original identifiers, paths and metric names as-is, no secrets / PII):
...
- **Upstream hint:** if an approved rule is not project-specific (it would improve every project of its kind), say so — it belongs in a shared ruleset maintained separately, through that ruleset's normal change process; `LLM-RULES.md` stays the project-local layer either way.

## note <text>

Record one friction signal without running the pass: a single self-contained memory note prefixed `rule-friction:` — target behavior / rule, what happened, date. Follow `.kilo/rules/project-memory.md` for the write provider and dated local fallback when no provider can save. Confirm in one line; no other writes.
...
- The command writes **only** `LLM-RULES.md` (plus the `memory.md` migration and `remember` notes described above) — never `AGENTS.md`, `USER-RULES.md`, `.kilo/rules/**`, installed rule copies, or `.dev.env`.
