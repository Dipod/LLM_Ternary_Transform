...
Control the terse `caveman` answer style. Canonical behaviour of the style and its modes — the `caveman` skill (`.kilo/skills/caveman/SKILL.md`); the `CAVEMAN` key, its values and default — `.kilo/rules/dev-standards-env.md → "CAVEMAN — caveman auto-activation"` (installed copies; match by file name per the path convention in `AGENTS.md`). Load the skill before acting.

Parameters, classes and defaults — `.kilo/rules/dev-standards-env.md §1`; Defaulted keys are never asked for.
