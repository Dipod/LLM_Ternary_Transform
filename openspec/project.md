...
**Phase 1 Stage A (2026-09-25): structural redundancy and post-hoc removal — VOID, no verdict issued.** Change `phase1-redundancy-cartography` implements the three-axis cartography and the combined probe in `phase1_compression/`. Spec R4 voided the stage: the 8-bit group-128 sanity baseline moved perplexity by +0.167 % (clip-searched) and +0.153 % (pure absmax), above the pre-registered 0.1 %. The map (`results/p1_map_2026-09-25e/`) finds **no removable blocks** (cheapest single-layer KL 0.1105 > budget 0.10), **no removable heads** (5 % → KL 2.89), about **5 % of FFN dimensions** inside budget, and about **32 least-significant residual directions per band**; post-hoc N:M masking of the ternary factors is catastrophic (`E_x` 0.22–0.56 against the 0.036 baseline), so the 42 % zero valley does **not** extend post-hoc. End to end on `Qwen/Qwen3-8B` (WikiText-2 test, 65 536 tokens, reference perplexity 12.6398, peak VRAM 15.6 GB) removing 5–6 % of FFN dimensions is free in perplexity (12.5867, −0.42 %, at 1.034×) but the curve collapses between 6 % and 8 % (+5.24 % at 8 %, +67.73 % at 10 %); integer baselines at ≥ 2× all fail the ±0.5 % bar (4-bit +9.70 %, 5-bit +2.47 %, 6-bit +0.94 %). Post-hoc structural redundancy of this model is worth ~4 % of parameters, not 50 %. **The void was resolved as a threshold defect, not a harness defect:** a mainstream library 8-bit arm (`torchao` 0.13.0 `int8_weight_only`, group 128) costs +0.38 %, worse than the hand-rolled RTN (+0.167 %), so the 0.1 % sanity tolerance sits below what any 8-bit weight-only scheme achieves on this model; recalibrating it is a threshold change and needs its own decision (rule R-005). The library 4-bit arm cannot run on this GPU (needs AMD CDNA2+; the RX 7900 XTX is RDNA3). **The direction is closed by the user's decision (2026-09-25): post-hoc structural removal is not pursued further.** Full record, limitations and the surviving Stage B candidates (MoE expert removal, ternary codec, scale testing): `results/decision_report.md`.
